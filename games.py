...
def battleship():
    ...
    score = score + score5
    cur.execute(
        "INSERT INTO score (USER_ID, GAME_ID, SCORE_VALUE) VALUES (%s, battleship, %s)",
        (username, score)
    )
    con.commit()
    print("Score saved!")
...
def periodic():
    ...
    print(f"Quiz Over! Your score: {score2}/{questions}")
    score = score2 * 10
    if score == questions:
    ...
    cur.execute(
        "INSERT INTO score (USER_ID, GAME_ID, SCORE_VALUE) VALUES (%s, periodic quiz, %s)",
        (username, score)
    )
    con.commit()
    print("Score saved!")
...
def rpg():
    ...
    cur.execute(
        "INSERT INTO score (USER_ID, GAME_ID, SCORE_VALUE) VALUES (%s, RPG arena, %s)",
        (username, score)
    )
    con.commit()
    print("Score saved!")
...
def blackjack():
    ...
    cur.execute(
        "INSERT INTO score (USER_ID, GAME_ID, SCORE_VALUE) VALUES (%s, blackjack, %s)",
        (username, score)
    )
    con.commit()
    print("Score saved!")
